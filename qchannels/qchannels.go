...
func New(bot *traqwsbot.Bot, use []string) *QChannels {
	q := &QChannels{bot: bot}
	q.RefreshBimap()
	return q
}
...
func (q *QChannels) RefreshBimap() error {
...
func (q *QChannels) GetChannelID(path string) *string {
	channelID, exists := q.channelPathID[path]
	if exists {
		return &channelID
	} else {
		return nil
...
func (q *QChannels) GetChannelPath(id string) *string {
	channelPath, exists := q.channelIDPath[id]
	if exists {
		return &channelPath
	} else {
		return nil
...
func (q *QChannels) GetParent(channelID string) *string {
	parentID, exists := q.idTree[channelID]
	if exists {
		return &parentID
	} else {
		return nil
